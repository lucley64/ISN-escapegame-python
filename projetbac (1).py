...
pasDeplacement=2
deplacement = [pasDeplacement, pasDeplacement]
noir = 100, 100, 255
...
controle=pygame.transform.scale(controle,(tailleFenetre))
...
clé2=pygame.image.load("petiteclé.jpg")
...
def main(mur, item1, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11, item12):
    ...
    while not done:
        y=time.time()-montemps
        timer = font.render(str(time.localtime(y)[3]-1),'h',str(time.localtime(y)[4]),'min',str(time.localtime(y)[5]),'s', True, (255, 255, 255))
        ecran.blit(timer,(1300,50))
        pygame.display.flip()
        time.sleep(0.016666667)
        if time.localtime(y)[3]-1==1:
            done = True
        print (time.localtime(y)[3]-1,'h',time.localtime(y)[4],'min',time.localtime(y)[5],'s')
        if (time.localtime(y)[4]==1 or time.localtime(y)[4]==10 or time.localtime(y)[4]==15 or time.localtime(y)[4]==20 or time.localtime(y)[4]==25 or time.localtime(y)[4]==30 or time.localtime(y)[4]==35 or time.localtime(y)[4]== 40 or time.localtime(y)[4]==45 or time.localtime(y)[4]==50 or time.localtime(y)[4]==55) and time.localtime(y)[5]==0:
            if not item1:
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1 :
            ...
            if event.pos[0]>800 and event.pos[1]>150 and event.pos[0]<1125 and event.pos[1]<800 and mur == 'avt' and item5 and door:
                if fenetre("Vous avez les clés de la porte.\nL'ouvrir?", 'porte', 'oui', 'non', 'la porte est maitenant ouverte', 'ok')== 'yes':
                    door=False
                    son_porte=pygame.mixer.Sound(b'porte cellule.wav')
                    son_porte.play()
